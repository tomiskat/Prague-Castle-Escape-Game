    ...
    import escape.game.managers.LocationManagerImpl
    import escape.game.models.LocationData
    ...
    import escape.game.utils.FirebaseManager
    ...
    class GameViewModel(
        ...
        private fun getGameData(): Map<String, Any> {
            return mapOf(
                "duration" to getGameDuration(),
                "mistakes" to _wrongAnswersCount,
            )
        }
        ...
        private fun onStoringFailure(exception: Exception) {
            if (exception.message != null && exception.message.equals(Constants.NAME_ALREADY_EXISTS)) {
